...
class LineTanh(nn.Module):
    def __init__(self):
        super(LineTanh, self).__init__()
        self.alpha = nn.Parameter(torch.tensor(1.0))  # 初始化 alpha 参数

    def forward(self, x):
        out = x * (1/self.alpha)  # 线性变换
        out = torch.clamp(out, -1, 1)  # 使用 clamp 进行裁剪
        return out

class FeatureExpander(nn.Module):
    def __init__(self, in_ch, out_ch, flat_in, flat_out):
        super(FeatureExpander, self).__init__()
        self.flatten = nn.Flatten(start_dim=2)  # 将 (64, 10, 25) 展平为 (64, 250)
        self.fc = nn.Linear(flat_in, flat_out)  # 将 (64, 250) 转换为 (64, 36)
        self.flat_out = flat_out
        self.out_ch = out_ch
        self.in_ch = in_ch
        self.conv = nn.Conv1d(in_ch, out_ch*in_ch, kernel_size=1)  # 用于扩展维度，生成 192 倍通道

    def forward(self, x):
        # 输入形状 (B, 64, 10, 25)
        B, C, H, W = x.shape
        x = self.flatten(x)  # (B, 64, 250)
        x = self.fc(x)  # (B, 64, 36)

        # 调整形状以适应 1D 卷积操作
        # x = x.permute(0, 2, 1)  # (B, 36, 64)
        x = self.conv(x)  # (B, 192*64, 36)
        
        # 调整形状以匹配目标输出
        # x = x.view(B, self.flat_out, self.out_ch, self.in_ch).permute(0, 2, 3, 1)  # (B, 192, 64, 36)
        x = x.view(B*self.out_ch, self.in_ch, self.flat_out, 1)
...
class Attention(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,
            multi_head=3, down_scale=1, attention_times=1, input_shape=None, cfg=None):
        ...
        if input_shape is None:
            ...
            self.over_attention = None
            self.initModuleBlocks(input_shape, down_scale)


    def initModuleBlocks(self, input_shape, down_scale):
        _init_block = torch.zeros((1,self.out_channels,input_shape[0]//down_scale, input_shape[1]//down_scale))
    ...
    def getScaleConv(self):
        ...
        if self.down_scale == 1:
            return nn.Conv2d(self.in_channels, self.in_channels, 1, 1, 0, bias=False) \
                    , nn.Conv2d(self.in_channels, self.in_channels, 1, 1, 0, bias=False)
        for _ in range(self.down_scale-1):
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, input):
        ...
        for i in range(self.multi_head):
            ...
            soft_addressing = self.softmax(self.layernorm(self_score))
