...
class CEmbadding_Conv(nn.Module):
    def __init__(self, in_ch, out_ch, in_shape, out_shape, scale_num=1):
        ...
        self.embadding = nn.Sequential(nn.Flatten(start_dim=-2)
                                       , nn.Linear((in_shape[0]*in_shape[1])//(2**(2*scale_num))
...
if __name__ == "__main__":
    ...
    cnn = CNNTransformer(3, (32, 32), 256, (16, 16), 1, 4, 3)
    # cnn(x).to('cuda')
    cnn.to('cuda')
    from torchsummary import summary
    from ptflops import get_model_complexity_info
    print(summary(cnn, input_size=(3, 32, 32)))  # 假设输入尺寸为 (3, 32, 32))
    macs, params = get_model_complexity_info(cnn, (3, 32, 32), as_strings=True, print_per_layer_stat=True)
